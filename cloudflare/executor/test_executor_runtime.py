...
from executor_runtime import CentralClient, ExecutorRuntime
...
class FakeCentral(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if self.path.startswith("/api/executor/v1/claim"):
...
class TestExecutorRuntime(unittest.TestCase):
    ...
    def test_full_loop_claim_ack_complete_upload(self):
        ...
        self.assertIn("/api/executor/v1/ack", paths)
        self.assertIn("/api/executor/v1/credentials", paths)
        self.assertIn("/api/executor/v1/artifacts", paths)
        # complete 在 credentials 之后（凭据只在租约内使用）
        self.assertLess(paths.index("/api/executor/v1/credentials"),
    ...
    def test_unknown_task_type_reports_task_invalid(self):
        ...
        runtime._execute({"task_id": "t-x", "lease_id": "l-x", "task_type": "nope.type"})
        completes = [b for p, b in FakeCentral.seen if p.endswith("/complete")]
        self.assertEqual(completes[-1].get("error_code"), "TASK_INVALID")
    ...
    def test_handler_crash_maps_to_retry(self):
        ...
        FakeCentral.seen.clear()
        runtime._execute({"task_id": "t-c", "lease_id": "l-c", "task_type": "demo.echo"})
        completes = [b for p, b in FakeCentral.seen if p.endswith("/complete")]
