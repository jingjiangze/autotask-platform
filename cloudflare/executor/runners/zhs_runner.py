...
from runners.chaoxing_runner import PYEXE, build_chaoxing_env
...
def run_zhs(payload: dict[str, Any], ctx) -> dict[str, Any]:
    """知到刷课：run_zhs.py full <ids>。凭据租约解封；payload.qr=true 时走扫码 cookies（§65）。"""
    courses = str(payload.get("courses") or "")
    timeout = int(payload.get("timeout_seconds") or 1800)
    if payload.get("qr"):
        # 扫码单：cookies 已由 zhs_qr.run 写入订单目录，引擎以 FUCKCOURSE_COOKIES 免登录
        from runners.zhs_qr_runner import _cookies_path
        order_id = str(payload.get("order_id") or "").strip() or ctx.task_id
        cpath = _cookies_path(order_id)
        if not os.path.isfile(cpath):
            raise RuntimeError("QR cookies missing —— 请在订单详情重新扫码")
