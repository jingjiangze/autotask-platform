...
def load_config_from_root():
    """从根 config.json 加载超星配置, 没有则用默认值并写回"""
    ...
    section = root.get("chaoxing", None)

    # 补齐缺失的 section/key
    changed = False
    if section is None:
        section = json.loads(json.dumps(CHAOXING_DEFAULTS))
        changed = True
    else:
        for sn in ("tiku", "notification"):
            if sn not in section:
                section[sn] = {}
                changed = True
        if "common" not in section:
            section["common"] = {}
            changed = True
        for k, v in CHAOXING_DEFAULTS["common"].items():
            if k not in section["common"]:
                section["common"][k] = v
                changed = True
    ...
    tiku = section.get("tiku", {})
    notification = section.get("notification", {})

    for key in ("speed", "jobs"):
        if key in common:
            common[key] = float(common[key]) if key == "speed" else int(common[key])

    if "notopen_action" not in common:
        common["notopen_action"] = "retry"
    if "use_cookies" in common:
        common["use_cookies"] = str_to_bool(common["use_cookies"])
    if "tree_view" in common:
        common["tree_view"] = str_to_bool(common["tree_view"])
...
def init_config():
    ...
    if root_config is not None:
        ...
        if args.list:
            common["course_list"] = [item.strip() for item in args.list.split(",") if item.strip()]
        if args.jobs:
            common["jobs"] = args.jobs
        if args.notopen_action:
            common["notopen_action"] = args.notopen_action
        if args.use_cookies:
            common["use_cookies"] = True
    else:
        common, tiku, notification = build_config_from_args(args)

    # 凭据优先级：命令行 > 环境变量(WK_ACCOUNT/WK_PASSWORD，平台注入，避免命令行明文) > 配置文件
    if not common.get("username"):
        common["username"] = os.environ.get("WK_ACCOUNT", "") or ""
    if not common.get("password"):
        common["password"] = os.environ.get("WK_PASSWORD", "") or ""
    return common, tiku, notification
...
def init_chaoxing(common_config, tiku_config):
    ...
    # 如果没有提供用户名密码，且 cookies 也不可用，则从命令行获取
    from api.cookies import use_cookies as load_cookies
...
def process_chapter(chaoxing: Chaoxing, course:dict[str, Any], point:dict[str, Any], speed:float) -> ChapterResult:
    ...
    # 用正确的章节编号覆盖服务器文件名（后者序号残缺，如1.6→6-, 2.2→2）
    for job in jobs:
...
def print_course_tree(point_list):
    """打印课程完整章节目录树"""
    chapters = point_list.get("chapters", [])
    if not chapters:
        return
    ...
    for chapter in chapters:
        ...
        print(f"{PREFIX}__{title}"[:TERM_COLS])
        for section in chapter.get("sections", []):
            _print_section(section, depth=2)
    print(f"{'=' * 50}\n")
...
def _print_section(section, depth):
    """递归打印章节树"""
    prefix = PREFIX * depth
    status = " [已完成]" if section.get("has_finished") else ""
    print(prefix)
    print(f"{prefix}__{section['title']}{status}"[:TERM_COLS])
    for child in section.get("children", []):
        _print_section(child, depth + 1)
